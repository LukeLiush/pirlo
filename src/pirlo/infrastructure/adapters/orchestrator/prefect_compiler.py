...
from pirlo.core.models.blueprint import BlueprintError, BlueprintNode, PlaybookBlueprint, PlaybookOutput
...
class PrefectCompiler:
    ...
    @classmethod
    def compile(cls, blueprint: PlaybookBlueprint) -> Callable[..., Awaitable[PlaybookOutput | None]]:
        ...
        async def prefect_master_flow(**workflow_kwargs: object) -> PlaybookOutput | None:
            ...
            for blueprint_node in blueprint.nodes:
                ...
                for param_name, param_binding in blueprint_node.param_bindings.items():
                    parent_future: PrefectFuture[PlaybookOutput] = futures[param_binding.source_node_id]
                    parent_result: PlaybookOutput = await parent_future.result()
                    resolved_kwargs[param_name] = getattr(
                        parent_result, param_binding.source_field, None
                    )

                playbook_cls: type[Pitch[PlaybookOutput]] = cls._resolve_playbook_class(blueprint_node.playbook_name)

                @flow(name=f"Subflow: {blueprint_node.playbook_name}")
                async def subflow_runner(**kwargs: object) -> PlaybookOutput:
                    instance: Pitch[PlaybookOutput] = playbook_cls()
                    playbook_result: PlaybookOutput | RunResult[PlaybookOutput] = await instance.play(**kwargs)
                    return playbook_result.data if isinstance(playbook_result, RunResult) and playbook_result.data else cast(PlaybookOutput, playbook_result)

                parent_futures: list[PrefectFuture[PlaybookOutput]] = [
                    futures[parent_id] for parent_id in blueprint_node.depends_on
                ]
                prefect_future: PrefectFuture[PlaybookOutput] = await subflow_runner.submit(
            ...
            if blueprint.output_node_id and blueprint.output_node_id in futures:
                final_prefect_future: PrefectFuture[PlaybookOutput] = futures[blueprint.output_node_id]
                return await final_prefect_future.result()
            return None
