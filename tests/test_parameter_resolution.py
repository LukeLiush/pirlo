...
async def test_default_values_fallback(monkeypatch, tmp_path):
    ...
    assert res.data["task"] == "default-task"
    assert res.data["count"] == 5
    assert res.data["enabled"] is False
    assert res.data["items"] is None
    assert res.data["details"] is None
    assert res.data["path_val"] is None
    assert res.data["multi_env"] == "default-multi"
...
async def test_env_variables_resolution(monkeypatch, tmp_path):
    ...
    assert res.data["task"] == "env-task"
    assert res.data["count"] == 42
    assert res.data["enabled"] is True
    assert res.data["items"] == ["a", "b", "c"]
    assert res.data["details"] == {"foo": "bar"}
    assert res.data["path_val"] == Path("/var/log")
    assert res.data["multi_env"] == "value-b"
...
async def test_cli_overrides_env_and_default(monkeypatch, tmp_path):
    ...
    assert res.data["task"] == "cli-task"
    assert res.data["count"] == 100
    assert res.data["enabled"] is True
    assert res.data["items"] == ["x", "y"]
